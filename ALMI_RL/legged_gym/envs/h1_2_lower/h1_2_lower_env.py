...
class H1_2_WholeBody(LeggedRobot):
    ...
    def reset(self):
        ...
        obs, privileged_obs, _, _, _, upper_obs = self.step(torch.zeros(self.num_envs, self.num_actions, device=self.device, requires_grad=False), torch.zeros(self.num_envs, self.num_upper_actions, device=self.device, requires_grad=False))
        return obs, privileged_obs, upper_obs
    ...
    def get_upper_observations(self):
        return self.upper_obs_buf

    
    def step(self, actions, upper_actions):
        ...
        self.upper_actions = torch.clip(upper_actions, -clip_actions, clip_actions).to(self.device)
        upper_actions = self.upper_actions.clone()
        ...
        for _ in range(self.cfg.control.decimation):
            self.torques = self._compute_torques(self.actions, self.upper_actions).view(self.torques.shape)
        ...
        self.upper_obs_buf = torch.clip(self.upper_obs_buf, -clip_obs, clip_obs)

        if self.privileged_obs_buf is not None:
            self.privileged_obs_buf = torch.clip(self.privileged_obs_buf, -clip_obs, clip_obs)
        return self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras, self.upper_obs_buf

    
    def _compute_torques(self, actions, upper_actions):
        ...
        upper_action_scaled = upper_actions * self.cfg.control.action_scale # policy *  actions_scaled


        wholebody_action = torch.cat([actions_scaled, upper_action_scaled], dim=-1)
    ...
    def reset_idx(self, env_ids):
        ...
        self.upper_last_actions[env_ids] = 0.
    ...
    def _init_buffers(self):
        ...
        self.num_upper_actions = 9
        self.upper_actions = torch.zeros(self.num_envs, self.num_upper_actions, dtype=torch.float, device=self.device, requires_grad=False)
        self.upper_last_actions = torch.zeros(self.num_envs, self.num_upper_actions, dtype=torch.float, device=self.device, requires_grad=False)

        self.num_upper_obs = 68
        self.upper_obs_buf = torch.zeros(self.num_envs, self.num_upper_obs, device=self.device, dtype=torch.float)
    ...
    def post_physics_step(self):
        ...
        self.compute_upper_observations()

        self.last_actions[:] = self.actions[:]
        self.last_dof_vel[:] = self.dof_vel[:]
        self.last_root_vel[:] = self.root_states[:, 7:13]
        
        self.upper_last_actions[:] = self.upper_actions[:]
    ...
    def compute_upper_observations(self):
        """ Computes observations
        """

        ref_dof = self.default_dof_pos[:, -9:] + (self.motion_buffer[torch.arange(self.num_envs), self.episode_length_buf] - self.default_dof_pos[:, -9:]) * (self.arm_weight if self.cfg.asset.arm_curriculum else 1.0)

        self.upper_obs_buf = torch.cat((  self.base_ang_vel  * self.obs_scales.ang_vel,
                                    self.projected_gravity,
                                    ref_dof,
                                    (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos,
                                    self.dof_vel * self.obs_scales.dof_vel,
                                    self.upper_actions, ### 区别
                                    self.left_sin_phase,
                                    self.right_sin_phase
                                    ),dim=-1)
