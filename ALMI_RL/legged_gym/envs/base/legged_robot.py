...
class LeggedRobot(BaseTask):
    ...
    def get_rpy(self):
        self.roll_pitch_yaw = torch.zeros(self.num_envs, 3)
        for i in range(self.num_envs):
            self.roll_pitch_yaw[i,:] = torch.tensor(gymapi.Quat(self.base_quat[i][0],
                                                                self.base_quat[i][1],self.base_quat[i][2],self.base_quat[i][3]).to_euler_zyx())
    def step(self, actions):
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            self.q_list.append(self.dof_pos[0,:].detach().cpu().numpy())
    ...
    def _process_dof_props(self, props, env_id):
        ...
        # print(props["friction"], props["damping"], props["armature"])
