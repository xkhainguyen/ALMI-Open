...
def get_args_parser():
    ...
    ## optimization
    parser.add_argument('--total-epoch', default=100, type=int, help='number of total iterations to run')
    ...
    ## gpt arch
    parser.add_argument("--num-obs", type=int, default=83, help="num obs plus act")
    parser.add_argument("--seq-length", type=int, default=50, help="seq_len per window")
    parser.add_argument("--block-size", type=int, default=25, help="seq len")
    parser.add_argument("--max-motion-len", type=int, default=25, help="max motion len")
    ...
    parser.add_argument('--save-iter', default=1000, type=int, help='evaluation frequency')
    parser.add_argument('--seed', default=123, type=int, help='seed for initializing training. ')
    parser.add_argument("--if-maxtest", action='store_true', help="test in max")
    parser.add_argument("--pred-action", action='store_true', help="only pred action")
    parser.add_argument("--use-tokenizer", action='store_true', help="use tokenizer")
    parser.add_argument("--add-noise", action='store_true', help="add noise in obs-action in training")
    parser.add_argument("--pred-all", action='store_true', help="test in max")
    parser.add_argument('--pkeep', type=float, default=1.0, help='keep rate for gpt training')
    
    parser.add_argument('--device', type=str, default='cuda:0', help='training device')
    parser.add_argument('--use-tcn', action='store_true', help='if use tcn')
